-- Fix profile creation issue - CORRECTED VERSION
...
SELECT 
    ...
    COALESCE(au.raw_user_meta_data->>'role', 'tenant_user')::user_role as role
FROM auth.users au
LEFT JOIN public.profiles p ON au.id = p.id
WHERE p.id IS NULL;
...
RETURNS TRIGGER AS $$
BEGIN
    -- Insert profile with better error handling and proper type casting
    INSERT INTO public.profiles (id, email, full_name, role)
    VALUES (
        NEW.id,
        NEW.email,
